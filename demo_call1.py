...
def make_demo_call(phone_number=None):
    ...
    try:
        ...
        # Monitor call status to detect NO_PICKUP
        try:
            # Wait for a short time to let the call establish
            time.sleep(30)  # Wait 30 seconds
            
            # Check call status
            call_status = client.calls(call.sid).fetch().status
            logger.info(f"Call status after 30 seconds: {call_status}")
            
            # If call status indicates no pickup, categorize it
            if call_status in ['no-answer', 'busy', 'failed', 'canceled']:
                # Import the categorization service
                from services.call_categorization_service import CallCategorizationService, CallCategory
                
                # Create an instance of the service
                categorization_service = CallCategorizationService()
                
                # Categorize the call as NO_PICKUP
                categorization_service.categorize_no_pickup(call.sid, phone_number)
                logger.info(f"Categorized call {call.sid} as NO_PICKUP - status: {call_status}")
                
                # Export categorized calls to JSON
                export_path = "categorized_calls.json"
                categorization_service.export_to_json(export_path)
                logger.info(f"Exported categorized calls to {export_path}")
        except Exception as e:
            logger.error(f"Error monitoring call status: {str(e)}")
